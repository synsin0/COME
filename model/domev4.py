...
class BEVDropout_layer(nn.Module):
    ...
    def forward(self, BEV_layout):
        ...
        BEV_layout = 1-self.maxpool(1-BEV_layout)
...
class DomeV4(nn.Module):
    ...
    def forward(self, 
        ...
        # random drop
        if self.control_dropout_prob > 0 and self.training:
        # if self.control_dropout_prob > 0:
            for control in controls:
                random_list = random.sample(range(control.shape[1]), int(control.shape[1] * self.control_dropout_prob ))
                control[:,random_list,:] = 0
