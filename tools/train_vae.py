...
def main(local_rank, args):
    ...
    while epoch < max_num_epochs:
        ...
        for i_iter, (input_occs, target_occs, metas) in enumerate(train_dataset_loader):
            ...
            use_pose_condition = torch.rand(1) < cfg.get('p_use_pose_condition',0)
            result_dict = my_model(x=input_occs, metas=metas,use_pose_condition=use_pose_condition)
            ...
            for loss_input_key, loss_input_val in cfg.loss_input_convertion.items():
                input_=result_dict[loss_input_val]
                if 'temperal_mask' in result_dict:
                    t_mask=result_dict['temperal_mask']
                    if input_.dim()==4:
                       t_mask= t_mask.unsqueeze(1)
                    input_*=t_mask
                loss_input.update({
                    loss_input_key: input_})
