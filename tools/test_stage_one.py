...
def main(local_rank, args):
    ...
    CalMeanIou_sem_input = multi_step_MeanIou(unique_label, cfg.get('ignore_label', -100), unique_label_str, 'sem', times=cfg.get('eval_length', 10))
    ...
    with torch.no_grad():
        for i_iter_val, (input_occs, target_occs, metas) in tqdm(enumerate(val_dataset_loader),total=len(val_dataset_loader)):
            ...
            # val_miou_input, _ = CalMeanIou_sem_input._after_step(input_occs, target_occs,log_current=True)
            ...
            # pred[~stage1_invisible_mask] = 17
            # result_dict['target_occs'][~stage1_invisible_mask] = 17
    ...
    val_miou, _ = CalMeanIou_sem._after_epoch()
    val_iou, _ = CalMeanIou_vox._after_epoch()
    # val_miou_input, _ = CalMeanIou_sem_input._after_epoch()
    ...
    # logger.info(f'Input val iou is {val_miou_input}')
